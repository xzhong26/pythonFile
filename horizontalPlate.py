...
# csv_writer.writerow(["ts_code", "index", "trade_date", "open", "high", "low", "close", "pre_close", "change", "pct_chg", "vol", "amount", "status", "avg5hp"])
    ...
    for key in ts_codes:
        ...
        for n, row in enumerate(tsk):
            ...
            if n >= 4:
                ...
                print(key, row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], status, "xxxx")
        # break
f.close()